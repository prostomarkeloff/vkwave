...
class Auth:
    ...
    data: Dict[str, Union[int, str]] = {"grant_type": "password"}
    ...
    def __init__(
        ...
        self.data.update({"client_id": self.client_id, "client_secret": self.client_hash})
    ...
    async def auth(
        ...
        if two_auth_code:
            self.data.update({"code": two_auth_code})

        response = await self.client.request_json(method="post", url=self.auth_url, data=self.data)
